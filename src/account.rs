...
impl Account {
    ...
    pub fn generate_ecdsa_certificate(
        ...
        ssl::generate_certificate(self, not_before, not_after, server_address, false)
    }
    ...
    pub fn generate_ed25519_certificate(
        ...
        ssl::generate_certificate(self, not_before, not_after, server_address, true)
    }
...
mod tests {
    ...
    fn test_client_ecdsa_certificate() {
        let account = get_random_account();
        let account_address = utils::format_scalar(account.address());
    ...
    fn test_verify_client_ecdsa_certificate() {
        ...
        let der = account
            .generate_ecdsa_certificate(not_before, not_after, None)
            .unwrap();
        let verifier = Account::verify_ssl_certificate(der.as_slice(), now, None).unwrap();
