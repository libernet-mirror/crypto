...
pub trait Hash {
    /// Hashes two input scalars and returns a single (uniformly distributed) output scalar.
    fn hash(input1: Scalar, input2: Scalar) -> Scalar;

    /// Hashes an arbitrary number of scalars.
    fn hash_many(input: &[Scalar]) -> Scalar;
}
...
impl<H: Hash> BatchProof<H> {
    ...
    pub fn verify(&self, commitment: &BatchCommitment) -> Result<()> {
        ...
        let values: Vec<Scalar> = self
            .paths
            .iter()
            .map(|leaf_proof| *leaf_proof.value())
            .collect();
...
pub struct BatchProver<H: Hash> {
    /// The size (degree+1) of each polynomial. Must be a power of 2.
    n: usize,
    /// Per-polynomial Merkle trees, each of length `2n` (leaves at `[0..n]`, nodes at `[n..2n]`,
    /// root at `(n - 1) * 2`).
...
impl<H: Hash> BatchProver<H> {
    ...
    pub fn new(values: Vec<Vec<Scalar>>) -> Self {
        ...
        let trees: Vec<Vec<Scalar>> = values
            .into_iter()
            .map(|mut p| {
                p.resize(2 * n - 1, Scalar::ZERO);
                merklify::<H>(&mut p, n);
                p
            })
            .collect();

        let roots: Vec<Scalar> = trees.iter().map(|tree| tree[(n - 1) * 2]).collect();
        let r = H::hash_many(
            std::iter::once(*DST)
    ...
    pub fn open_at(&self, index: usize) -> BatchProof<H> {
        assert!(index < self.n);
        let paths = self
            .trees
            .iter()
            .map(|tree| LeafProof::<H>::new(tree, self.n, index))
            .collect();
        BatchProof {
            index,
            paths,
...
mod tests {
    ...
    fn batch_open_verify_all<H: Hash>(values: Vec<Vec<Scalar>>) {
        let n = values
            .iter()
            .map(|p| p.len())
            .max()
        ...
        let expected: Vec<Vec<Scalar>> = values
            ...
            .collect();
        let prover = BatchProver::<H>::new(values);
        let commitment = prover.commit();
        for index in 0..n {
            let proof = prover.open_at(index);
            assert_eq!(proof.index(), index);
    ...
    fn test_batch_open_verify_one_poly() {
        let values = vec![vec![42.into()]];
        batch_open_verify_all::<Sha3Hash>(values.clone());
        batch_open_verify_all::<Poseidon2Hash>(values);
    }
    ...
    #[test]
    fn test_batch_open_verify_three_polys_non_power_of_two() {
        let values = vec![
            vec![10.into(), 20.into(), 30.into()],
            vec![40.into(), 50.into(), 60.into()],
            vec![70.into(), 80.into(), 90.into()],
        ];
        batch_open_verify_all::<Sha3Hash>(values.clone());
        batch_open_verify_all::<Poseidon2Hash>(values);
    }

    fn test_batch_rejects_wrong_commitment_impl<H: Hash>() {
        let polys1 = vec![vec![1.into(), 2.into()], vec![3.into(), 4.into()]];
        let polys2 = vec![vec![5.into(), 6.into()], vec![7.into(), 8.into()]];
        let prover1 = BatchProver::<H>::new(polys1);
        let prover2 = BatchProver::<H>::new(polys2);
    ...
    fn test_batch_rejects_wrong_commitment() {
        test_batch_rejects_wrong_commitment_impl::<Sha3Hash>();
        test_batch_rejects_wrong_commitment_impl::<Poseidon2Hash>();
    }

    fn test_batch_rejects_tampered_poly_value_impl<H: Hash>() {
        let values = vec![vec![1.into(), 2.into()], vec![3.into(), 4.into()]];
        let prover = BatchProver::<H>::new(values);
        let commitment = prover.commit();
        let mut proof = prover.open_at(0);
    ...
    fn test_batch_rejects_tampered_poly_value() {
        test_batch_rejects_tampered_poly_value_impl::<Sha3Hash>();
        test_batch_rejects_tampered_poly_value_impl::<Poseidon2Hash>();
    }

    fn test_batch_rejects_tampered_poly_path_impl<H: Hash>() {
        let values = vec![
            vec![1.into(), 2.into(), 3.into(), 4.into()],
            vec![5.into(), 6.into(), 7.into(), 8.into()],
        ];
        ...
        let commitment = prover.commit();
        let mut proof = prover.open_at(0);
        proof.paths[1].path[0] = 99.into();
        assert!(proof.verify(&commitment).is_err());
    }

    #[test]
    fn test_batch_rejects_tampered_poly_path() {
        test_batch_rejects_tampered_poly_path_impl::<Sha3Hash>();
        test_batch_rejects_tampered_poly_path_impl::<Poseidon2Hash>();
    }
}
